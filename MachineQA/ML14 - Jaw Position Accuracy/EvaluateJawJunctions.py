...
# pyinstaller -F --hiddenimport=pydicom.encoders.gdcm --hiddenimport=pydicom.encoders.pylibjpeg --clean EvaluateJawJunctions.py
...
def identify_individual_film_strip_edges(roi):
    ...
    # sanity check - expect film strips to be about 100 px wide. 
    # Sometimes user enters wrong number of film strips and in some instances the code will count that many.
    estimated_film_strips = round(roi.shape[0]/100)
    if estimated_film_strips != film_strip_count:
        print("The code could not find exactly {} exposed film strips.\nMake sure all exposed film are adjacent to eachother in the exposed scan.\n".format(expected_film_strip_count))
        sleep(1)
        return False, False
